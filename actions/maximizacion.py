def crear_tabla(tabla, desigualdades, restricciones):
    ...
    contador_artificial = sum(1 for desigualdad in desigualdades[:-1] if desigualdad == 1)
    ...
    for i in range(len(tabla)):
        nueva_tabla[i][len(tabla[0]):len(tabla[0]) + len(tabla_res[0])] = tabla_res[i]
...
def relacionar_tabla(tabla, desigualdades):
    for i in range(2, len(tabla)):
        if desigualdades[i] == 1:
            tabla[1] = [tabla[1][j] - tabla[i][j] for j in range(len(tabla[0]))]
...
def columna_pivote(tabla, fila_control):
    ...
    for i in range(len(tabla[0]) - 1):
        if tabla[fila_control][i] < tabla[fila_control][col_pivote]:
            col_pivote = i
    print(f"La columna pivote tiene el número {tabla[fila_control][col_pivote]}, posición {col_pivote + 1}")
    return col_pivote
...
def fila_pivote(tabla, columna_pivote):
    ...
    for i in range(len(cocientes)):
        if cocientes[i] < cocientes[menor] and cocientes[i] > 0:
            menor = i
            fila_pivote = i
    print(f"La fila pivote tiene el número {tabla[fila_pivote][columna_pivote]}, posición {fila_pivote + 1}")
    return fila_pivote
...
def dividir_por_pivote(tabla, fila_pivote, columna_pivote):
    ...
    tabla[fila_pivote] = [valor / num for valor in tabla[fila_pivote]]
...
def eliminar_filas(tabla, fila_pivote, columna_pivote):
    for i in range(len(tabla)):
        if i != fila_pivote:
            ...
            tabla[i] = [tabla[i][j] - multiplicador * tabla[fila_pivote][j] for j in range(len(tabla[0]))]
...
def maximizacion(tabla, desigualdades, restricciones, funcion_obj):
...
maximizacion(tabla, desigualdades, restricciones, funcion_obj)
